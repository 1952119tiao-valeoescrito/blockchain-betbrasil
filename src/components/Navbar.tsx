      ...
      <div className={`fixed inset-0 z-[110] bg-black/95 backdrop-blur-2xl transition-all duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full p-6">
          ...
          <div className="flex flex-col gap-8 text-center">
            <Link href="/como-funciona" onClick={() => setIsOpen(false)} className="text-2xl font-black text-white uppercase tracking-tighter">{t('how')}</Link>
            <Link href="/resultados" onClick={() => setIsOpen(false)} className="text-2xl font-black text-white uppercase tracking-tighter">{t('results')}</Link>
            <Link href="/premiacao" onClick={() => setIsOpen(false)} className="text-2xl font-black text-white uppercase tracking-tighter">{t('prizes')}</Link>
            <Link href="/inter-bet" onClick={() => setIsOpen(false)} className="text-2xl font-black text-yellow-500 uppercase tracking-tighter">{t('interbet')}</Link>
