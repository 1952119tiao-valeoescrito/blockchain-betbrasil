    ...
    <>
        ...
        <div className="container mx-auto px-4 h-full flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 shrink-0">
            ...
            <div className="flex flex-col text-left leading-none">
              <span className="text-white font-bold tracking-tighter text-base">BLOCKCHAIN <span className="text-yellow-500">BET</span></span>
              <span className="text-[10px] text-gray-400 font-bold tracking-[0.2em] uppercase">{t('subtitle')}</span>
            </div>
          </Link>

          {/* DESKTOP NAV - NOMES NOVOS APLICADOS */}
      ...
      <div className={`fixed inset-0 z-[110] bg-black/95 backdrop-blur-2xl transition-all duration-300 transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full p-8">
          <button onClick={() => setIsOpen(false)} className="self-end text-white p-2 mb-12"><X size={32} /></button>
          <div className="flex flex-col gap-10 text-center uppercase tracking-widest font-black">
