...
export default function ComoFuncionaPage() {
  ...
  // Componente da Grade Visual 25x25 com todos os 625 prognósticos
  const MatrixDisplay = () => (
    <div className="w-full max-w-5xl mx-auto my-12 p-1 bg-gradient-to-b from-white/10 to-transparent rounded-2xl border border-white/5 shadow-2-900/40 border border-white/5 p-8 rounded-[2rem] hover:border-yellow-500/30 transition-all group">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">{txl overflow-hidden">
...
('card2Desc')}</p>
            ...
            <React.Fragment key={`row-${row}`}>
              ...
              {Array.from({ length: 25 }, (_, col) => {
                ...
                return (
                  <div 
                    key={`cell-${x}-${y}`} 
                    className="aspect-square        <section className="bg-slate-900/60 border border-white/10 rounded-[3rem] p-8 md:p-16 mb-24 relative overflow-hidden text-left shadow-2xl">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-12 uppercase tracking-tighter italic flex items-center gap-4">
            <Search bg-white/5 border-[0.5px] border-white/10 hover:bg-yellow-500/20 hover:border-yellow-500/40 transition-all rounded-[1px] flex items-center justify-center group className="text-yellow-500" /> {t('auditTitle')}
          </h2>
          <div className="grid md:grid-cols-3 gap-12">
            {[1, 2, 3].map((step) => (
              <div key={step} className="space-y-4">"
                  >
                    <span className="text-[7px] md:text-[8px] text-gray-700 group-hover:text-yellow-200 transition-colors font-mono font-medium">
                      {x}/{y}
                    </span>
                  </div>
                );
                ...
                <div className="text-yellow-500 font-black text-4xl opacity-30 font-mono italic text-center md:text-left">0{step}</div>
      ...
      <div className="bg-black/40 py-3 text-[10px] text-gray-500 uppercase tracking-[0.4em] font-black text-center border-t border-white/5">
        Matriz Operacional: 25x25p className="text-gray-400 text-sm leading-relaxed">{t(`step${step}Desc`)}</p>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="text-center px-4">
            ...
            <div className="grid md:grid-cols-3 gap-8 text-left max-w-6xl mx-auto">
              <div className="bg-slate-900/80 p-8 rounded-[2rem] border border-white/10 hover:border-blue-500/40 transition-all group">
                ...
                <h3 className="text-white font-bold text-xl mb-4 uppercase leading-none tracking-tight">{ = 625 Prognósticos Reais
      </div>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#020617] text-slate-10t('Beginners.walletTitle')}</h3>
                <p className="text-sm text-gray-400 leading-relaxed">{t('Beginners.walletDesc')}</p>
              </div>
              <div className="bg-slate-900/80 p-8 rounded-[2rem] border border-white/0 font-sans selection:bg-yellow-500 selection:text-black">
      <Navbar />
      ...
      <main className="container mx-auto px-4 pt-32 pb-20 text-center">
        ...
        10 hover:border-green-500/40 transition-all group">
              ...
              <div className="bg-slate-900/80 p-8 rounded-[2rem] border border-white/10 hover:border-yellow-500/40 transition-all group">
                <BarChart className="text-yellow-500 mb-6 group-hover</header>

        {/* MATRIZ COM PROGNÓSTICOS */}
        <MatrixDisplay />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-24 mt-20 text-left">
          <div className="bg-slate-900/40 border border-white/5 p-8 rounded-[2rem] hover:border-blue-500/30 transition-all group">
            <Target className="text-blue-500 mb:scale-110 transition-transform" size={32} />
        ...
        <div className="text-center">
          <Link href="/inter-bet">
            <button className="bg-yellow-500 hover:bg-yellow-400 text-black font-black text-xl px-12 py-5 rounded-full shadow-6 group-hover:scale-110 transition-transform" size={40} />
            <-[0_0_50px_rgba(234,179,8,0.3)] transition-all transform hover:scale-105 flex items-center gap-3 mx-auto uppercase tracking-widest leading-none">
