...
export default function ComoFuncionaPage() {
  ...
  const MatrixDisplay = () => (
    <div className="w-full max-w-5xl mx-auto my-12 p-1 bg-gradient-to-b from-white/10 to-transparent rounded-2xl border border-white/5 shadow-2xl overflow-hidden">
      <div className="bg-[#0b0c10]/90 backdrop-blur-md overflow-x-auto p-4 md:p-8 custom-scrollbar text-center">
        <div className="grid grid-cols-[30px_repeat(25,1fr)] gap-1 min-w-[850px] mx-auto">
          ...
          {Array.from({ length: 25 }, (_, row) => (
            <React.Fragment key={`row-${row}`}>
              {/* Index Lateral */}
              <div className="text-[10px] text-gray-600 font-bold flex items-center justify-center uppercase font-mono text-center">
                {row + 1}
              </div>
              ...
              {Array.from({ length: 25 }, (_, col) => {
                ...
                return (
                  <div 
                    key={`cell-${x}-${y}`} 
                    className="aspect-square bg-white/5 border-[0.5px] border-white/10 hover:border-yellow-500/40 transition-all rounded-[1px] flex items-center justify-center group"
                  >
